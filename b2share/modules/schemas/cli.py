...
def community_schema_add_block_schema_version(verbose, community, block_schema_id, version=None):
    """Creates a new version of the community schema with this block_schema 
        ...
        thereby activates the community."""
    #fetch community
    comm = get_community_by_name_or_id(community)
    if not comm:
        raise click.BadParameter("There is no community by this name or ID: %s" 
    ...
    if not version < len(block_schema.versions):
        raise click.BadParameter("""BlockSchema %s has %d versions, you 
            specified version %d that does not exist""" % (block_schema_id, 
            len(block_schema.versions), version))
    if verbose:
        click.secho("Generating version URL for %s version %d" % 
            (block_schema_id, version))
    version_url = resolve_schemas_ref("$BLOCK_SCHEMA_VERSION_URL[%s::%d]" % 
        (block_schema_id, version))
    ...
    if not community_schema:
        ...
        community_schema_dict['properties'][str(block_schema_id)]= \
        {'$ref':version_url}
    #save
...
def community_schema_remove_block_schema_version(verbose, block_schema_id, version=None):
    ...
    #TODO decide whether this should be implemented?    